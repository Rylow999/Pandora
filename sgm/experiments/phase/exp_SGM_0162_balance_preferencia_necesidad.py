#!/usr/bin/env python3
"""exp_SGM_0162 — Integra las 3 piezas del Mundo Interno en subsistencia de Crafter.
...
def run(seed, n_lives=8):
    ...
    for vida in range(n_lives):
        ...
        for step in range(2000):
            ...
            accion_imag=None
            if hambre>0.4:
                # 3 PIEZAS DEL MUNDO INTERNO COORDINADAS con ARBITRO PREFERENCIA-NECESIDAD (0162):
                # (a) META: el arbitro elige entre lo que NECESITA (food si hambre critica) y lo
                # que VALORA (valencia si necesidad cubierta). La hambre manda sobre la valencia.
                meta_preferida = ag.elegir_meta(hambre, ['food'], ['food','wood','wood_pickaxe'])
                n_valoro += 1
                # (b) RAZONAMIENTO sobre la meta elegida (desde memoria episodica)
...
print("="*70)
print(" exp_SGM_0162 — MUNDO INTERNO INTEGRADO (memoria+imaginacion+valencia) en subsistencia")
print("="*70)
...
json.dump({
  ...
  "phase":"Fase 9 - balance preferencia-necesidad (arbitro elegir_meta) en subsistencia",
  "date":"2026-08-14",
  "mision":"refinar el balance entre valencia (preferencia) y homeostasis (necesidad): la hambre "
           "critica manda sobre lo que el agente valora; la valencia solo guia la exploracion cuando "
           "la necesidad esta cubierta. Corrige la obsesion por valencia del 0161.",
  "hypothesis":"con el arbitro elegir_meta, el agente PRIMERO sobrevive (come) y luego explora por "
               "preferencia -> mejora la subsistencia vs 0161 (que priorizaba la valencia estricta)",
  ...
  "script":"experiments/exp_SGM_0162_mundo_interno_integrado.py",
  "results_file":"results/results_exp_SGM_0162_mundo_interno_integrado.json",
