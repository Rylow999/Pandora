#!/usr/bin/env python3
"""exp_SGM_0150 — GRAFO DE CONOCIMIENTO DEL MUNDO (V2): el sustrato aprende COMPOSICION.
...
def correr_vida(ag, env, vida_i, max_p=1500):
    ...
    for step in range(max_p):
        ...
        # PRECONDICION ESPACIAL DE LA COMPOSICION (0150): el make_* necesita una MESA cerca
        # (nearby table). Detectar si hay mesa/pareja adyacente (usando world.nearby, la API
        # real de Crafter) y condicionar la curiosidad de composicion a que la precondicion
        # este o se dirija a crearla (place_table usa wood:2).
        try:
            mat_near, objs_near = env._world.nearby((px, py), 1)
            hay_mesa = 'table' in mat_near
        except Exception:
            mat_near, objs_near = [], []
            hay_mesa = False
        # CURIOSIDAD DIRIGIDA A COMPOSICION CON PRECONDICION (0150):
        # - si tengo RECURSOS + MESA -> muestrear el make_* que consume esos recursos (funciona).
        # - si tengo RECURSOS pero NO mesa -> suplir la precondicion: dirigir a place_table
        #   (si tengo wood>=2) o a recolectar wood, en vez de fallar make a ciegas.
        prefiere_comp=None
        for acc,recurso_neces in COMP_RECURSOS.items():
            if acc in probadas_comp: continue
            if all(inv.get(r,0)>0 for r in recurso_neces):
                # tengo los recursos para este make
                if hay_mesa:
                    prefiere_comp=acc   # mesa lista -> probar el make
                    sesgo_comp=0.9
                else:
                    # falta la mesa -> aprender la precondicion: dirigir a place_table
                    if inv.get('wood',0) >= 2:
                        prefiere_comp=8   # place_table (accion 8) usa wood:2
                        sesgo_comp=0.9
                    else:
                        prefiere_comp=None  # sin wood suficiente, que explore/recolecte
                break
        a=ag.step(sv,TODAS)
        # aplicamos el sesgo de curiosidad a composicion/precondicion (si el sustrato no eligio)
        if prefiere_comp is not None and a not in COMP_ACCIONES and (a!=8) and random.random()<0.35:
            a=prefiere_comp  # muestrear make (mesa lista) o place_table (precondicion)
        ...
        # registro de PRECONDICION: place_table y logros del mundo (place_table logrado)
        if a == 8:
            if info['achievements'].get('place_table', 0) > 0:
                descubrio_comp.append('place_table')
                print(f"    [PRECONDICION] vida {vida_i} paso {step}: place_table COLOCADA (cadena lista)")
...
print("="*70)
print(" exp_SGM_0150 — GRAFO CONOCIMIENTO V2: PRECONDICION ESPACIAL DE LA COMPOSICION")
print("="*70)
...
json.dump({
    "experiment_id":"exp_SGM_0150","experiment_name":"grafo_conocimiento_composicion_V2",
    ...
    "result":{"vidas":TODOS},
    "script":"experiments/exp_SGM_0150_grafo_conocimiento.py",
    "results_file":"results/results_exp_SGM_0150_grafo_conocimiento.json",
