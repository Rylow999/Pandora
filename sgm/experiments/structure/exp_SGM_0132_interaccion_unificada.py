#!/usr/bin/env python3
"""exp_SGM_0132 — INSTINTO DE INTERACCION unificado (hambre + defensa via 'do').
...
MECANISMO (core 0132): la pulsion a 'do' sube = max(hambre_real, amenaza) * fuerza,
...
def correr(seed, max_p=600):
    ...
    for step in range(max_p):
        ...
        # amenaza: hp perdido reciente + enemigo en el gradiente
        danio = max(0.0, (prev_hp - hp) / 10.0)   # hp bajo = dolor reciente
        g_enemigo, _ = gradiente(sem, px, py, ENEMIGO)
        amenaza_env = 1.0 if g_enemigo != (0, 0) else 0.0
        amenaza = max(danio * 2.0, amenaza_env)
        ...
        ag._gradiente_dir = g_enemigo; ag._gradiente_dist = 0
...
print("=" * 70)
print(" exp_SGM_0132 — Instinto de interaccion unificado (hambre + defensa via 'do')")
print("=" * 70)
for seed in [42, 7, 99]:
...
out = os.path.expanduser("~/vaults/vega-vault/NOUS/DSCN-G/EXPERIMENTS/SGM/"
                         "results/results_exp_SGM_0132_interaccion_unificada.json")
os.makedirs(os.path.dirname(out), exist_ok=True)
json.dump({
    "experiment_id": "exp_SGM_0132",
    "experiment_name": "instinto_interaccion_unificado",
    "phase": "Fase 8 - instinto de interaccion unificado (hambre + defensa via 'do')",
    "date": "2026-08-11",
    ...
    "script": "experiments/exp_SGM_0132_interaccion_unificada.py",
    "results_file": "results/results_exp_SGM_0132_interaccion_unificada.json",
    "variant_of": "exp_SGM_0131",
