#!/usr/bin/env python3
"""exp_SGM_0146 — REWARD SHAPING POR HITO + acople fino del 'do'.
...
R = {'cerca': 0.05, 'adyacente': 0.2, 'enfrente': 0.5, 'come': 1.0}
...
def comida_madura_adyacente_enfrente(world, px, py, facing):
    """Devuelve (adyacente|enfrente, tipo). Comida madura: cow o planta ripe.
    'enfrente' = en pos+facing (el do puede comerla)."""
    adyacente = False
    for obj in world.objects:
        ...
        if es == 'Cow' or (es == 'Plant' and obj.ripe):
            ...
            if (int(obj.pos[0]), int(obj.pos[1])) == (ex, ey):
                return 'enfrente', es
    return ('adyacente' if adyacente else 'nada'), None
...
print("=" * 70)
print(" exp_SGM_0146 — REWARD SHAPING POR HITO (mostrar cómo se hace, sin 1M pasos)")
print("=" * 70)
RESULTADOS = []
for seed in [42, 7]:
...
json.dump({
    ...
    "script": "experiments/exp_SGM_0146_reward_shaping.py",
    "results_file": "results/results_exp_SGM_0146_reward_shaping.json",
