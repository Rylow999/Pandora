#!/usr/bin/env python3
"""exp_SGM_0159 — Fase 9 en acción: memoria episódica + imaginación para mejorar la subsistencia.
...
- MEMORIA EPISODICA (0159a): el agente RECUERDA los eventos salientes (crafteo, comida) y al
  razonar la meta, busca primero en los recuerdos, no solo en el buffer plano.
- IMAGINACION (0159b): antes de ejecutar, el agente SIMULA (con el modelo del mundo) la
...
def run(seed, n_lives=8):
    ...
    for vida in range(n_lives):
        ...
        for step in range(2000):
            ...
            # IMAGINACION (0159b): si tengo hambre, imaginar que acciones podrian ayudar
            accion_imag = None
            if hambre > 0.4:
                ...
                if acc_raz is not None:
                    ...
                    # GATE explotacion-exploracion (0158-A): solo explotar (usar la
                    # imaginacion) si el modelo del mundo es CONFIABLE en este estado; si
                    # hay incertidumbre, explorar (dejar que la curiosidad decida novedad).
                    if (conf > 0 or bonus > 0) and ag.decidir_explotar(eq, acc_raz):
                        accion_imag = acc_raz
                        n_imagino += 1
            # decidir: usar la imaginacion (si el gate permitio explotar) o la curiosidad
...
print("="*70)
print(" exp_SGM_0159 — Fase 9 en accion: memoria episodica + imaginacion vs subsistencia")
print("="*70)
...
json.dump({
  ...
  "script":"experiments/exp_SGM_0159_memoria_imag_subsistencia.py",
  "results_file":"results/results_exp_SGM_0159_memoria_imag_subsistencia.json",
