#!/usr/bin/env python3
"""exp_SGM_0169 — DIAGNOSTICO del crafteo (un solo grafo, instrumentado).
...
def _sv(info, mesa_cerca=0.0):
    inv=info['inventory']
    sem=info['semantic']
    # 0169: senales perceptuales AGREGADAS al final del vector:
...
def run(seed, n_lives=8):
    ...
    for vida in range(n_lives):
        ...
        for step in range(2000):
            ...
            # 0169: SIN prefiere_comp (no hardcode del crafteo). El agente decide por su
            ...
            # decidir accion: LIBRE (el agente elige con su percepcion + aprendizaje)
            a = ag.step(sv, TODAS)
...
print("="*70)
print(" exp_SGM_0169 — PERCEPCION de la mesa: el crafteo emerge, sin hardcode")
print("="*70)
...
out=os.path.expanduser("~/vaults/vega-vault/NOUS/DSCN-G/EXPERIMENTS/SGM/"
                       "results/results_exp_SGM_0169_percepcion_mesa_crafteo.json")
json.dump({
  "experiment_id":"exp_SGM_0169","experiment_name":"percepcion_mesa_crafteo_emergente",
  "phase":"Fase 9 - percepcion dela mesa: el crafteo emerja del modelo del mundo (sin hardcode)",
  "date":"2026-08-15",
  "mision":"que el agente perciba la mesa (senal en sv) y aprenda por su modelo del mundo que make "
           "requiere mesa cerca - EMERGENTE, sin regla 've a la mesa'. Sin prefiere_comp (libre).",
  ...
  "result":{"diag":diag,"logros":logros,"pasos":total,
    ...
    "script":"experiments/exp_SGM_0169_percepcion_mesa_crafteo.py",
    "results_file":"results/results_exp_SGM_0169_percepcion_mesa_crafteo.json",
