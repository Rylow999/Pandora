#!/usr/bin/env python3
"""exp_SGM_0153 — LIBERTAD + CONOCIMIENTO DEL MUNDO + MUNDO PERSISTENTE + 30min.
...
def run(seed, t_max_pasos=15000, n_lives=8):
    ...
    for vida in range(n_lives):
        ...
        for step in range(2000):
            ...
            if 'make_wood_pickaxe' in nuevo_inv and paso_make_primero is None:
                paso_make_primero = step  # memoria: paso del primer crafteo (acelera entre vidas)
                # 0153-A: consolidar el HITO (accion_make -> nodo del pico) directo, memoria fuerte
            ...
            if info['achievements'].get('eat_cow',0)>0 and paso_eat_primero is None:
                ...
                # 0153-A: consolidar el HITO de comer (do -> nodo0 comida) directo
...
print("="*70)
print(" exp_SGM_0153 — ROBUSTEZ: cadena de subsistencia en MULTIPLES seeds + vidas")
print("="*70)
TODAS_SEEDS = []
for seed in [42, 7, 2024]:
    res, logros, pasos = run(seed, n_lives=8)
...
out=os.path.expanduser("~/vaults/vega-vault/NOUS/DSCN-G/EXPERIMENTS/SGM/"
                       "results/results_exp_SGM_0153_robustez_seeds.json")
os.makedirs(os.path.dirname(out),exist_ok=True)
json.dump({
    "experiment_id":"exp_SGM_0153","experiment_name":"robustez_multi_seed_cadena_subsistencia",
    "phase":"Fase 8 - ROBUSTEZ: cadena de subsistencia en multiples seeds + vidas largas",
    "date":"2026-08-14",
    "mision":"Confirmar que el HITO 0151 (subsistencia emergente) es ROBUSTO: la cadena "
             "collect->place_table->make->eat_cow se repite en MULTIPLES seeds (no solo seed 42).",
    "config":{"D":D,"N_NODES":N_NODES,"n_lives":8,"max_pasos_vida":2000,"seeds":[42,7,2024],
              "mundo_persistente":True,"conocimiento_mundo":"recetas make/place como repertorio"},
    "result":{"seeds":TODAS_SEEDS},
    "script":"experiments/exp_SGM_0153_robustez_seeds.py",
    "results_file":"results/results_exp_SGM_0153_robustez_seeds.json",
    "variant_of":"exp_SGM_0150",
